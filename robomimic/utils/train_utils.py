...
from robomimic.state_infuse.describer import Describer
...
TASK_LENGTH_MAPPER = pickle.load(open('task_length.pkl', 'rb'))
...
def sigmoid_shape(x, alpha=4):
    """
    Maps a value from [0, 1] to a sigmoid-shaped curve within [0, 1].

    Args:
        x (float or torch.Tensor): Input value(s) in the range [0, 1].
        alpha (float): Controls the steepness of the sigmoid curve.
                       Higher values make the curve steeper.

    Returns:
        torch.Tensor: Transformed value(s) in the range [0, 1].
    """
    # Shift and scale x to the range [-1, 1] before applying sigmoid
    x = (x * 2 - 1) * alpha  # Maps x from [0, 1] to [-alpha, alpha]
    # Apply sigmoid and scale back to [0, 1]
...
def run_rollout(
    ...
    if not env_state:
        ...
        ob_dict = env.reset_to(env_state)
    ...
    smoothed_complete_rate = 0.0  # Or some reasonable initial estimate
    momentum = 0.7  # Smoothing factor; adjust this as needed

    # for step_i in range(horizon): #LogUtils.tqdm(range(horizon)):
    for step_i in LogUtils.tqdm(range(int(horizon))):
        ...
        if batched:
            ...
            if with_progress_correct:
                ...
                task_emb = task_emb.repeat(1, 1)
                
                
                current_eef = ob_dict['robot0_base_to_eef_pos'][-1]
                current_quat = ob_dict['robot0_base_to_eef_quat'][-1]
                current_rpy = tf.Rotation.from_quat(current_quat.reshape(-1, 4)).as_euler('xyz')[0]
                current_gripper = ob_dict['robot0_gripper_qpos'][-1]
                hand_image = hand_images[-1]
                
                img_np = hand_image.transpose(1, 2, 0)
                ...
                # # _horizon = min(600, horizon)
                # # complete_rate = min(step_i / _horizon, 1.0)
                ...
                # # complete_rate_from_model = torch.mean(complete_rate_by_model).detach().cpu().numpy().item()

                # #
                # # if task_str not in TASK_LENGTH_MAPPER:
                # #     raise TypeError(f'Invalid task string: {task_str}')

                task_prior_length = TASK_LENGTH_MAPPER[task_str] if task_str in TASK_LENGTH_MAPPER else horizon * 0.6

                # standard_horizon = 500
                # horizon_factor = min(standard_horizon / horizon, 1) ** 0.3
                complete_rate= min(step_i / task_prior_length, 1.0)
                #
                # print('complete_rate_from_model: ', complete_rate_from_model)
                # print('complete_from_step: ', complete_from_step)

                # complete_rate = complete_rate_from_model
                # smoothed_complete_rate = momentum * smoothed_complete_rate + (1 - momentum) * complete_rate

                # complete_rate = max(smoothed_complete_rate, complete_from_step)
                # complete_rate = complete_rate[0][0].cpu().detach().numpy()

                if not hasattr(policy.policy, "prev_image"):
                    policy.policy.prev_image = None
                    policy.policy.acc_sim = 0
                    policy.policy.last_action = "VLM does not provide next action"

                    from robomimic.state_infuse.cal_similarity import image_features
                    policy.policy.image_features = image_features
                    policy.policy.clip_model, policy.policy.clip_preprocess = clip.load("ViT-B/32", device=policy.policy.device)
                    from robomimic.state_infuse.describer import Describer
                    policy.policy.describer = Describer(device=policy.policy.device)

                prev_image = policy.policy.prev_image
                acc_sim = policy.policy.acc_sim
                last_action = policy.policy.last_action
                image_features = policy.policy.image_features
                clip_model = policy.policy.clip_model
                clip_preprocess = policy.policy.clip_preprocess
                describer = policy.policy.describer

                if prev_image is None:
                    policy.policy.prev_image = PIL_img
                    acc_sim = 2.0  
                else:
                    feat_now = image_features(PIL_img, clip_model, clip_preprocess, policy.policy.device)
                    feat_past = image_features(prev_image, clip_model, clip_preprocess, policy.policy.device)
                    sim = feat_now @ feat_past.T  
                    acc_sim += (1 - sim)
                    # print(f'acc_sim: {acc_sim}')

                if acc_sim > 0.1:
                    condition, next_action = describer.describe(
                        task=task_str,
                        pil_img=PIL_img,
                        eef=current_eef,
                        rpy=current_rpy,
                        gripper_distance=current_gripper
                    )
                    acc_sim = 0  
                    policy.policy.last_action = next_action
                    save_dir = "./rollout_execution_record"
                    ...
                    # print(f"[Step {step_i}] [Describer] {next_action}")
                else:
                    next_action = last_action  
                    # print(f"[Step {step_i}] [Cached] {next_action}")
                # condition, next_action = policy.policy.describer.describe(
                #         task=task_str,
                #         pil_img=PIL_img,
                #         eef=current_eef,
                #         rpy=current_rpy,
                #         gripper_distance=current_gripper
                #     )
                # print(f"[Step {step_i}] Next Action: {next_action}")
                ...
                #                                                 )
                # print(f'complete_rate: {complete_rate} - next_action: {next_action}')
                ...
                x = (complete_rate_ratio, task_emb, next_action_emb_from_clip)

                state_emb = policy.policy.predict_boosted_state_mapping(
                    x, batch_size=1, timestep=10, finetune=False
                )
...
def rollout_with_stats(
    ...
    for env, horizon in zip(envs, horizon_list):
        ...
        if video_dir is not None:
            ...
            base_str = "_epoch_{}_baseline.mp4".format(epoch) if epoch is not None else "baseline.mp4"
            video_path = os.path.join(video_dir, "{}{}".format(env_name, video_str))
            base_video_path = os.path.join(video_dir, "{}{}".format(env_name, base_str))
        ...
        baseline_video_writer = None
        if write_video:
            print("video writes to " + video_path)
            env_video_writer = imageio.get_writer(video_path, fps=20)
            baseline_video_writer = imageio.get_writer(base_video_path, fps=20)
            ...
            try:
                ...
                if for_compare:
                    rollout_info = run_rollout(
                        ...
                        render=render,
                        use_goals=use_goals,
                        video_writer=baseline_video_writer,
                        task_str_writer=task_str_writer,
                        video_skip=video_skip,
                        terminate_on_success=terminate_on_success,
                        with_progress_correct=False,
