...
def train(config, device):
    ...
    model.initial_ensemble_state_mapper()
    # else:
    # state_mapping_model = None

    # model.state_mapping_model = state_mapping_model

    if config.experiment.baseline_ckpt_path is not None:
        baseline_ckpt_dict = maybe_dict_from_checkpoint(ckpt_path=config.experiment.baseline_ckpt_path)
        baseline_model.deserialize(baseline_ckpt_dict["model"])

    if config.experiment.state_mapping_ckpt_path is not None and config.algo.progress_dim_size > 0:
        print('loading state mapping model from {}'.format(config.experiment.state_mapping_ckpt_path))
        model.load_ensemble_state_mapping(
            epoch=config.experiment.state_mapping_epoch,
            path=config.experiment.state_mapping_ckpt_path
        )
    ...
    for epoch in range(0, config.train.num_epochs + 1): # epoch numbers start at 1
        ...
        if config.experiment.rollout.enabled and (epoch > config.experiment.rollout.warmstart) and rollout_check:
            ...
            rollout_model_baseline = RolloutPolicy(
                baseline_model,
                obs_normalization_stats=obs_normalization_stats,
                action_normalization_stats=action_normalization_stats,
                lang_encoder=lang_encoder,
            ...
            all_rollout_logs, video_paths = TrainUtils.rollout_with_stats(
                ...
                baseline_policy=rollout_model_baseline,
                envs=env_iterator(),
        ...
        if should_save_ckpt:    
            ...
            if config.algo.progress_dim_size > 0:
                print('saving state mapping model')
                model.save_ensemble_state_mapping(
                    epoch=epoch,
                    path=os.path.join(ckpt_dir, 'ensemble_state_mapping')
                )
